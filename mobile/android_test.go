...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", gfl.LightScryptN, gfl.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.flagman", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-flagman issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gfl-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/flagman/gfltest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for gfl and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.flagman", "github.com/getflagman/go-flagman/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gfl.aar"), "gfl.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "flagman", "gfltest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
