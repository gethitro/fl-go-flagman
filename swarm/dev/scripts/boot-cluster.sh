...
#
# The cluster contains a bootnode, a gfl node and multiple swarm nodes, with
...
# static gfl configuration
gfl_IP="192.168.33.3"
gfl_RPC_PORT="8545"
gfl_RPC_URL="http://${gfl_IP}:${gfl_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gfl and
# swarm nodes
start_bootnode() {
...
# start_gfl_node starts a gcm node with --datadir pointing at <base-dir>/gfl
# and a single, unlocked account with password "gfl"
start_gfl_node() {
  local dir="${base_dir}/gfl"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gfl account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${gfl_IP}"
    --rpcport   "${gfl_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gfl" "${gfl_IP}" "$(which gfl)" ${args[@]}
}
...
create_account() {
  ...
  gfl --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
